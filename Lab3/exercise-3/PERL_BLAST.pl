...
for my $i (0 .. $#data_s)
{
    my %kmers_with_location = findKmerWithFirstLocation($data_s[$i]);

    while ((my $key, my $value) = each(%kmers_with_location))
    {
...
#     my @cache_list = \$hash_table{$kmer_key};
#     # my $baibaicha = {};
#     # print(ref($hash_table));
#     # my $aaaa->isa($hash_table{$kmer_key});
#     # print("CACHED LIST: $aaaa");
#     my $record = $cache_list[0];
#     print($record->record::getStringID);
#     # my @cache_list = $hash_table{$kmer_key};
#     #
#     # for (my $i = 0; $i < length(@cache_list); $i++) {
#     #     my $record = $hash_table{$kmer_key}->[$i];
